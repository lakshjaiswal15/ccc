...
export function ComplaintSystem({
  ...
  const handleVote = async (complaintId: string) => {
    ...
    const updatedComplaints = complaints.map((complaint) => {
      if (complaint.id === complaintId) {
        ...
        if (updatedComplaint.userVote === "up") {
          ...
          // Add upvote
          updatedComplaint.upvotes += 1;
          updatedComplaint.userVote = "up";
        }

        // Ensure votes never go below 0
        updatedComplaint.upvotes = Math.max(
          0,
          updatedComplaint.upvotes,
        );
    ...
    try {
      ...
      await api.voteComplaint(complaintId, "up");
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
        <div>
          <h2 className="text-lg sm:text-xl text-dark-title">
      ...
      <div className="gradient-card p-3 sm:p-4 rounded-lg border-blue-300/30 text-foreground flex items-start space-x-3">
        ...
        <p className="text-sm">
          Complaints are visible to all residents and
          management. Please provide accurate information.
        </p>
      </div>
          ...
          {complaints.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-muted-foreground mt-2">
                Be the first to submit a complaint to help
            ...
            complaints.map((complaint) => (
              <div
                key={complaint.id}
                className="gradient-card p-3 sm:p-4 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
              >
                <div>
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 space-y-2 sm:space-y-0">
                    <div className="flex-1">
                      <h3 className="mb-1 text-sm sm:text-base text-foreground">
                        {complaint.title}
                      </h3>
                      <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm text-muted-foreground mb-2">
                        <Badge
                          variant="outline"
                          className="text-xs gradient-card-solid border-card-border"
                        >
                          {complaint.category}
                        </Badge>
                    ...
                    <div className="flex items-center space-x-2 self-start">
                      <div
                        className={`px-2 py-1 rounded text-xs border flex items-center space-x-1 ${getStatusColor(complaint.status)}`}
                      >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 space-y-2 sm:space-y-0">
                    <div className="flex items-center space-x-2">
                      ...
                      <div className="flex items-center gradient-card-solid rounded-lg p-1 border border-card-border">
                        ...
                        <span
                          className={`px-2 sm:px-3 text-xs sm:text-sm font-semibold min-w-[1.5rem] sm:min-w-[2rem] text-center ${complaint.upvotes > 0 ? "text-orange-400" : "text-muted-foreground"}`}
                        >
                          {complaint.upvotes}
                        </span>
                      </div>
                    ...
                    {complaint.status === "resolved" && (
                      <Badge
                        variant="default"
                        className="bg-green-500/20 text-green-400 border border-green-400/50 text-xs"
                      >
                        Resolved
                      </Badge>
                    )}
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex space-x-2">
                        ...
                        <Button
                          ...
                          disabled={
                            submittingComment ===
                              complaint.id ||
                            !commentTexts[complaint.id]?.trim()
                          }
                      ...
                      <div className="space-y-3">
                        {complaint.comments.map((comment) => (
                          <div
                            key={comment.id}
                            className="gradient-card-solid rounded-lg p-3 border border-card-border hover:bg-white/5 transition-colors"
                          >
      ...
      <div className="gradient-card p-3 sm:p-4 rounded-lg">
        <h3 className="mb-2 text-sm sm:text-base text-foreground">
          Complaint Guidelines
        </h3>
        <ul className="text-xs sm:text-sm text-muted-foreground space-y-1">
          <li>
            • Provide specific details and location for faster
            resolution
          </li>
          <li>
            • Use upvotes to show support and prioritize
            community concerns
          </li>
          <li>
            • Add comments to share additional information or
            updates
          </li>
        </ul>
      </div>
